...
class PromiseLoss(tf.keras.losses.Loss):
    def __init__(self, reduction=tf.keras.losses.Reduction.NONE, name='PromiseLoss'):
        super().__init__(reduction=reduction, name=name)

    def get_config(self):
        config = super(PromiseLoss, self).get_config()
        return config

    def call(self, alpha1, alpha2, beta1, beta2):
        def cost_promise(y_true, y_pred, alpha1, alpha2, beta1, beta2):
            ...
            intermediate_loss = tf.math.add(
                tf.math.multiply(tf.math.multiply(tf.math.add(tf.constant(1.0, tf.float64), s), alpha1),
                                 tf.math.multiply(e1,
                                                  tf.math.exp(tf.math.truediv(y_true, tf.constant(60.0, tf.float64))))),
                tf.math.multiply(tf.math.multiply(tf.math.subtract(tf.constant(1.0, tf.float64), s), alpha2),
                                 tf.math.multiply(e2, tf.math.exp(
                                     tf.math.subtract(tf.constant(1.0, tf.float64), tf.math.truediv(y_true,
                                                                                                    tf.constant(60.0,
                                                                                                                tf.float64)))))))
...
def o2a_model_fn(features_regressor, LEAKY_RELU_ALPHA, LEARNING_RATE, LOSS='mae',
    ...
    def model_fn(features, labels, mode):
        ...
        promise_loss = PromiseLoss()

        if LOSS == 'custom':

            loss_fn = promise_loss.call(tf.constant(ALPHA_1, tf.float64),
...
def o2d_model_fn(features_regressor, LEAKY_RELU_ALPHA, LEARNING_RATE, LOSS='mae',
                 ALPHA_1=1.0, ALPHA_2=1.0, BETA_1=1.0, BETA_2=2.0):
    def model_fn(features, labels, mode):
        features = {feat_name: tf.expand_dims(features[feat_name], -1) for feat_name in features_regressor}
        input_layer = tf.concat([features[feat_name] for feat_name in features], axis=-1)
        ...
        # eval metrics
        eval_metric_ops = {
            "mae_O2D_accurate": tf.compat.v1.metrics.mean_absolute_error(labels=labels, predictions=logits)}
        return tf.estimator.EstimatorSpec(mode=mode, loss=loss, eval_metric_ops=eval_metric_ops,
                                          predictions=predictions)
